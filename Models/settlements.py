...
def all_country_value(field_data, field_type):
    ...
    if field_type == 'Type':
        field_value = field_data.get('type') or (field_data.get('Typ').replace('Bestellung', 'Order').replace('Erstattung', 'Refund') if field_data.get('Typ') else field_data.get('Typ')) \
